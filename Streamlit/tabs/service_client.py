...
def note_attente(df_order, df_order_review):
    ...
    # Utilisez np.round pour arrondir les valeurs à des entiers
...
def wordcloud_commentaires(df_serviceclientfinal):
    ...
    # téléchargé la liste de stopwords
    # téléchargé la liste de stopwords
    nltk.download('stopwords')
    nltk.download('punkt')
    
    #text = re.sub(r'\W+', ' ', text)  # Remove non-alphabetic characters
    # Échantillon aléatoire de 10000 commentaires
    ...
    with st.expander("Voir explication"):
        st.write("""
            Vous trouverez ci-dessus un wordcloud sur un échantillon de 1000 commentaires pris aléatoirement parmis votre base de données.
                 ...
                 Note : il nous ai impossible de traduire les mots pour un si grand jeu de données. 
        """)
...
def wordcloud_decembre(df_serviceclientfinal):
    ...
    # Assuming dec_2018_comments_unique is your DataFrame with 'translated_comment' column
    ...
    with st.expander("Voir explication"):
        st.write("""
            Vous trouverez ci-dessus un wordcloud sur l'ensemble des données de décembre 2016. Cette analyse s'est effectuée suite
                 à une baisse importante constaté sur la note moyenne pour ce mois-ci (note de 2.1).
        """)
...
def meilleurs_notes(df_serviceclientfinal):
    ...
    with st.expander("Voir explication"):
        st.write("""
            Vous trouverez ci-dessus les 10 catégories de produits qui se sont vus attribués les 10 meilleures notes en moyenne.
                 Les notes allant de 4.64 à 4.30/5. 
        """)
...
def note_moyenne(df_serviceclientfinal):
    ...
    with st.expander("Voir explication"):
        st.write("""
            Vous trouverez ci-dessus l'évolution de la note moyenne par mois d'oct. 2016 à août 2018. On remarque que cette note oscille 
            ...
            pourquoi ce mois-ci à une note moyenne si faible afin que ce ne se reproduise pas dans le futur.""")
...
def top_flop(df_serviceclientfinal):
    ...
    with st.expander("Voir explication"):
        st.write("""
            Vous trouverez ci-dessus les 10 catégories de produits qui se sont vus attribués les 10 plus faibles notes en moyenne.
...
def tokenize_comment(comment):
    ...
    re.sub(r'\W+', ' ', token.lower())  # Transformation for each token
    for token in tokens                 # Iterating over each token in the 'tokens' list
        if token.lower() not in stopwords_pt # Condition: Exclude tokens found in the stopwords set
    ]
